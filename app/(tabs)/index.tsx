...
import { WARDROBE_STORAGE_KEY, wardrobeFreshness, type SavedWardrobePiece } from "@/lib/wardrobe-storage";
import { loadLocalWardrobeContext, mergeOwnedItemNames } from "@/lib/wardrobe-context";
...
export default function HomeScreen() {
  ...
  const [savedWardrobe, setSavedWardrobe] = useState<SavedWardrobePiece[]>([]);
  const [wardrobeSavedAt, setWardrobeSavedAt] = useState<string>();
  const [removedWardrobeNames, setRemovedWardrobeNames] = useState<string[]>([]);
  const startStyling = trpc.styling.start.useMutation();

  useEffect(() => {
    loadLocalWardrobeContext(() => AsyncStorage.getItem(WARDROBE_STORAGE_KEY)).then(({ pieces, savedAt, removedNames }) => { setSavedWardrobe(pieces); setWardrobeSavedAt(savedAt); setRemovedWardrobeNames(removedNames); });
  }, []);
  ...
  async function handleBuild() {
    ...
    const session = await startStyling.mutateAsync({
      intent: { mode: "occasion", request, occasion: occasion.toLowerCase(), aesthetic: "chic but relaxed", budgetCents: Math.max(1, Number(budget) || 180) * 100, ownedItems: mergeOwnedItemNames(owned, savedWardrobe, removedWardrobeNames) },
    });
